...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for dict in args.input:
            paths = os.listdir(dict)
        for path in tqdm.tqdm(paths, disable=not args.output):
            # use PIL, to be consistent with evaluation
            img = read_image(os.path.join(dict, path), format="BGR")
            img_id = os.path.basename(path)
            start_time = time.time()
            predictions, visualized_output = demo.run_on_image(img, img_id)
            print(predictions)
            logger.info(
                "{}: {} in {:.2f}s".format(
                    path,
            ...
            if args.output:
                if os.path.isdir(args.output):
                    ...
                    # out_filename = os.path.join(args.output, file_name+'_part'+ext)
                    out_filename = os.path.join(args.output, file_name+ext)
                else:
